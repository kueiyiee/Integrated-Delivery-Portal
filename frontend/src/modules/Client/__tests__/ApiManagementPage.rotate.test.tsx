...
describe('ApiManagementPage webhook rotate', () => {
  ...
  it('creates a webhook from the add webhook form with validation', async () => {
    const api = await import('../../../api');
    vi.mocked(api.api.post).mockImplementation((url: string, payload?: any) => {
      if (url === '/v1/client/api-management/webhooks') {
    ...
    await userEvent.type(screen.getByLabelText(/Provider/i), 'billing');
    await userEvent.type(screen.getByLabelText(/Webhook name/i), 'Billing Sync');
    await userEvent.type(screen.getByLabelText(/Target URL/i), 'https://example.com/webhooks/billing');
    await userEvent.type(screen.getByLabelText(/Events/i), 'delivery.created,delivery.updated');
    await userEvent.click(screen.getByRole('button', { name: /Add webhook/i }));

    await waitFor(() => expect(screen.getByText(/Webhook created successfully/i)).toBeInTheDocument());
  });
