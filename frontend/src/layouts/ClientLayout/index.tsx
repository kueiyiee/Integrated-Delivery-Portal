...
const baseNavigationItems = [
  { to: '/client', label: 'Operations Hub', icon: Gauge },
  { to: '/client/deliveries', label: 'Delivery Operations', icon: Truck },
];

const managerNavigationItems = [
  { to: '/client/reports', label: 'Insights & Reports', icon: ChartNoAxesCombined },
  { to: '/client/documents', label: 'Document Library', icon: FileText },
  { to: '/client/api-management', label: 'Developer Hub', icon: Webhook },
  { to: '/client/company', label: 'Organization Profile', icon: Building2 },
];
...
export function ClientLayout() {
  ...
  const [sidebarMode, setSidebarMode] = useState(() => getInitialSidebarMode('client_sidebar_mode') as SidebarMode);
  ...
  const breadcrumbItems = useMemo(() => {
    if (!currentItem) {
      return [{ label: 'Client Portal', to: '/client' }];
    }
    ...
    return [{ label: 'Client Portal', to: '/client' }, { label: currentItem.label }];
