...
import { Building2, LayoutDashboard, LogOut, Menu, PanelLeftClose, PanelLeftOpen, ShieldCheck, UserCog, Webhook, X } from 'lucide-react';
...
const navigationItems = [
  { to: '/admin', label: 'Enterprise Overview Dashboard', icon: LayoutDashboard },
  { to: '/admin/companies', label: 'Organization Management Center', icon: Building2 },
  { to: '/admin/api-keys', label: 'API Integration Management', icon: Webhook },
  { to: '/admin/reports', label: 'Audit & Intelligence Center', icon: ShieldCheck },
  { to: '/admin/profile', label: 'Administrator Profile & Security', icon: UserCog },
];
...
export function AdminLayout() {
  ...
  const [sidebarMode, setSidebarMode] = useState<SidebarMode>(() => getInitialSidebarMode('admin_sidebar_mode'));
  ...
  const breadcrumbItems = useMemo(() => {
    ...
    return [{ label: 'SYSTEM ADMINISTRATION CONSOLE', to: '/admin' }, { label: currentItem.label }];
  }, [currentItem]);
