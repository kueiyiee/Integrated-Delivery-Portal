...
import { api } from '../api';
import { getRequestErrorMessage } from '../services/auth';
...
export default function VerifyPage({ token }: { token?: string }) {
  ...
  const scannerSupported = useMemo(() => {
    return Boolean(typeof window !== 'undefined' && 'BarcodeDetector' in window && navigator?.mediaDevices?.getUserMedia);
  }, []);
  ...
  useEffect(() => {
    ...
    void api.get(`/v1/public/reports/verify/${verificationToken}`)
      ...
      .catch((err) => {
        ...
        const st = err?.response?.data?.status ?? (isTimeout ? 'temporary_error' : 'not_found');
        setStatus(st);
        setError(err?.response?.data?.message ?? getRequestErrorMessage(err, 'Verification could not be completed right now. Please try again.'));
      });
              ...
              <div style={{ display: 'grid', gap: 20, textAlign: 'center', padding: 28 }}>
                ...
                <div>
                  <div style={{ fontSize: 24, fontWeight: 700, color: '#0f172a' }}>
                    {status === 'not_found' ? 'Document not found' : status === 'temporary_error' ? 'Temporary service issue' : 'Verification issue'}
                  </div>
