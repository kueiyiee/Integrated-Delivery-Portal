...
import { forgotPassword, resolveAuthErrorMessage, PROFESSIONAL_ERROR_MESSAGES } from '../../services/auth';
...
export default function ForgotPage() {
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSuccess(response.message || 'A secure password reset code has been sent to your registered email address.');
      navigate(`/reset-password?email=${encodeURIComponent(email.trim())}`);
    } catch (err: any) {
      setError(resolveAuthErrorMessage(err, 'forgot-password', 'We were unable to process your password recovery request. Please try again shortly.'));
    } finally {
      setLoading(false);
    }
  };
